...
- For collector-facing Inline, default to the registered canonical shell and compact raw-percent carriage: self-contained `data:image/*` plus raw-percent HTML. Store original binary image bytes once; at the final JSON image field assemble the `data:image/<type>;base64,` header, canonical Base64 of those exact bytes, and the JSON delimiter/footer. This is media carriage, not a second Base64 storage object or whole-document encoding. GIFs are direct `data:image/gif;base64,...` from the exact source and are never wrapped in SVG, regenerated, resized, gateway-backed, or replaced by a placeholder such as `AA==`. External resolvers, IPFS/HTTP locators, `web3://`, and complete-HTML Base64 require an explicit reviewed exception.
