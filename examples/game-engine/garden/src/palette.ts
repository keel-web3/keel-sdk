...
// their own colours (entity spec.colours), so the seed dresses them.
//
...
// Ported from the proof of concept's projects/garden/palette.js.

import { oklch } from "@keel/game-engine/core";
import type { RGB } from "@keel/game-engine/core";
import type { PaletteFn, World, WorldMaterial } from "@keel/game-engine/world";
...
const around = ([L, C, h]: Oklch, n = 6, shift = 0): RGB[] => ramp(n, h + shift, Math.max(C, 0.02), Math.max(0.12, L - 0.42), Math.min(0.97, L + 0.16), 14);

export const MATERIALS: readonly WorldMaterial[] = [
...
/** The hero's material roles, by material name. */
export const HERO_MATERIALS: Readonly<Record<string, string>> = { dark: "dark", fur: "fur", furAlt: "fur", cloth: "cloth", clothAlt: "accent", accent: "accent", blush: "paint", hair: "dark" };
/** Animal `i`'s (1-based) material roles: its own fur. */
export const animalMaterials = (i: number): Record<string, string> => ({ dark: "dark", fur: `critter-${((i - 1) % 4) + 1}`, blush: "paint", cloth: "accent", accent: "accent", hair: "dark" });

// The colours the hero and the animals suggest (their specs), or a default.
function wearOf(world: World): { fur: Oklch; cloth: Oklch; accent: Oklch; critters: Oklch[] } {
  const hero = world.entities.get("hero")?.spec.colours;
  const pet = (i: number): Oklch => world.entities.get(`animal-${i}`)?.spec.colours.fur ?? [0.65, 0.1, 55 + i * 40];
  return {
...
interface Mood {
  stone: number; paving: number; water: number; waterL: [number, number]; sky: number; skyL: [number, number];
  grass: number; grassL: [number, number]; glow: number; paint: number; petal: number; chroma?: number;
}
// Each palette: the world's mood as hues and lightness ranges; the characters' ramps ride along.
function build(mood: Mood): PaletteFn {
  return (world) => {
...
export const PALETTES: Readonly<Record<string, PaletteFn>> = {
