...
//
// It is the world runtime's second project, and it wires nothing by hand:
// the generator proposes every choice through the settings (so locks hold),
// brains drive the animals, the default systems do the rest. The same
// function builds it in Node (tests) and in the page (index.html).
//
//   const world = gardenWorld({ seed: "3", width: 128, height: 128, locks: "tag:animal/species=cat" });
//   world.simulate(5); world.frame();

import { createWorld } from "../../src/world/world.js";
import { defineObject, socketOf } from "../../src/object/object.js";
import { yawTo } from "../../src/core/frame.js";
import { animalMaterials, HERO_MATERIALS, MATERIALS, PALETTES } from "./palette.js";
...
export function gardenGenerator(g, world) {
  ...
  const hedge = (id, pos, w, d) => g.place("pillar", { id, tags: ["hedge"], ctx: { w, d, h: hh }, pos, on: "auto" });
  const len = P - 0.4;
  const side = P - 1.4;
  for (const s of [-1, 1]) {
    for (const t of [-1, 1]) {
  ...
  for (const [i, s] of [[1, 1], [2, -1]]) {
    const pos = [s * (P - 0.55), plazaTop, s * diag * (P - 0.55)];
  ...
  const spots = [];
  for (const [nx, nz] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) for (const o of [-0.5, 0.5]) spots.push([nx * (P - 0.9) + nz * o * P, nz * (P - 0.9) + nx * o * P]);
  ...
  const [sx, sz] = [[1, 0], [-1, 0], [0, 1], [0, -1]][sside];
  const spos = [sx * (P + 2.4) + sz * 1.6, top, sz * (P + 2.4) + sx * 1.6];
  ...
  const corner = g.choose("crates.corner", [[1, 1], [1, -1], [-1, 1], [-1, -1]]);
  ...
  for (let i = 1; i <= crates; i += 1) {
    ...
    const jitter = [S.between(-0.08, 0.08), S.between(-0.08, 0.08)];
    const yaw = S.between(-0.4, 0.4);
    if (stack) {
      // (On top of the last one -- if it clears everything else up there.)
      const p = last.transform.pos;
      const pos = [p[0] + jitter[0], p[1] + last.def.meta.tall, p[2] + jitter[1]];
      if (g.fits(def, { pos, yaw, margin: 0.05, ignore: [last.id] })) { last = g.place(def, { id, tags: ["crate"], pos, yaw, on: "auto" }); continue; }
    }
    for (let k = 0; k < 12; k += 1) {
      const pos = [corner[0] * (P + 3.2 + S.between(0, 2.2)), top, corner[1] * (P + 3.2 + S.between(0, 2.2))];
  ...
  for (let k = 0; k < 40 && nb < bushes; k += 1) {
    const pos = [B.between(-G + 1.4, G - 1.4), top, B.between(-G + 1.4, G - 1.4)];
  ...
  for (let i = 1; i <= animals; i += 1) {
    ...
    for (let k = 0; k < 30; k += 1) {
      const p = [A.between(-G + 2, G - 2), top, A.between(-G + 2, G - 2)];
...
const toward = (from, to) => { const dx = to[0] - from[0]; const dz = to[1] - from[2]; const l = Math.hypot(dx, dz) || 1; return [dx / l, dz / l, l]; };

/**
 * An animal: idle a while, wander to a point on the lawn, sometimes sit.
 * Its numbers come from its own stream (world.rng("mind:<id>")), so adding
 * an animal never changes what another does.
 */
// (The one the frame camera is showing stands still for it.)
const posing = (world, ent) => world.camera?.mode === "frame" && world.state.camera?.showing === ent.id;

export function wander(world, ent, dt) {
  const m = ent.mind;
  ...
  if (world.time >= m.until) {
    ...
    m.state = R.weighted([["walk", 5], ["idle", 3], ["sit", 2]]);
 ...
 */
export function stroll(world, ent, dt) {
  const m = ent.mind;
  const R = world.rng(`mind:${ent.id}`);
  ...
  if (m.state === "sit") {
    ...
    const s = socketOf(world.objects.get(m.bench), "seat");
    ent.hold = null;
    ent.frozen = false;
 ...
 * scanlines, grade, rim ... -- is one of the renderer's src/fx passes.)
 */
export const FX = {
...
/**
 * gardenWorld({ seed, width, height, locks, scene }) -> a generated world, with
 * GARDEN's materials, palettes, brains, fx, particles and a leaf-fall system.
 *   locks   "scope/key=value;..." (src/world/settings.js parseLocks)
 *   scene   a scene scope's settings
 */
export function gardenWorld({ seed = "1", width = 128, height = 128, locks = "", scene = {}, input } = {}) {
  ...
  world.on("landed", (e, w) => w.particles.emit("dust", e.at, { count: Math.min(8, 2 + Math.round(e.speed)), spread: 1.2 }));
