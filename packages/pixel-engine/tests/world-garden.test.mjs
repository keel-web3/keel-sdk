// projects/garden on the world runtime: 100 seeds lay out with nothing
// overlapping and nothing floating; locks hold without moving the rest; the
// animals and the hero get about; the frame camera shows each one's front.

import { test } from "node:test";
import assert from "node:assert/strict";
import { frontOf } from "../src/core/frame.js";
import { restsOn, worldAabb } from "../src/object/object.js";
import { gardenWorld } from "../projects/garden/garden.js";

// Boxes overlap when they share volume (touching is not overlapping).
const EPS = 1e-6;
const overlap = (a, b) => a[0] < b[3] - EPS && a[3] > b[0] + EPS && a[1] < b[4] - EPS && a[4] > b[1] + EPS && a[2] < b[5] - EPS && a[5] > b[2] + EPS;
...
test("100 garden seeds: no two objects overlap, and every object rests on what it stands on", () => {
  for (let seed = 1; seed <= 100; seed += 1) {
    ...
    assert.ok(objs.some((o) => o.id.startsWith("bench")) && objs.some((o) => o.id.startsWith("crate")), `seed ${seed}: benches and crates`);
    // Resting: settled onto a support, and still on it -- the floor under the lawn, the lawn, the plaza, a crate.
    for (const o of objs) {
      const r = w.rests[o.id];
      assert.ok(r && r.rests, `seed ${seed}: ${o.id} rests`);
      const support = r.support === "floor" ? 0 : w.objects.get(r.support);
    ...
    for (let i = 0; i < objs.length; i += 1) {
      for (let k = i + 1; k < objs.length; k += 1) {
        ...
        if (w.rests[a.id].support === b.id || w.rests[b.id].support === a.id) continue;
...
test("garden: the animals wander and the hero strolls; nothing is left in the air", () => {
  ...
  for (let i = 0; i < 12; i += 1) {
    w.simulate(1);
    for (const e of w.entities.values()) states.add(`${e.id}:${e.mind.state}`);
  }
  for (const e of w.entities.values()) {
    const moved = Math.hypot(e.body.pos[0] - start.get(e.id)[0], e.body.pos[2] - start.get(e.id)[2]);
...
test("garden frame mode: the camera goes round the entities, each shown from its front", () => {
  ...
  for (let i = 0; i < 4 * 12; i += 1) {
    w.simulate(0.125);
    const id = w.state.camera?.showing;
    if (!id || w.state.camera.at < 1) continue; // (let the shot settle on its subject)
    seen.add(id);
    const body = w.entities.get(id).body;
    const to = [w.camera.eye[0] - body.pos[0], w.camera.eye[2] - body.pos[2]];
...
test("garden frames at 32, 64, 128 and 256: the same world, the target rules' style", () => {
  for (const [n, screen] of [[32, 2], [64, 4], [128, 4], [256, 8]]) {
