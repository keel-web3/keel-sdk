    ...
    messages: [{
      ...
      content: {
        ...
        text: [
          ...
          "Assume the creator is not a developer. For supplied assets, inventory actual types, digests and dependencies; preserve originals; discover reusable selected-chain objects; measure lossless storage and decoder costs; and automatically choose resource boundaries and compatible compression. Do not ask the user to choose modules or codecs. Ask only about creative intent, rights, budget or visible quality tradeoffs; never silently apply lossy conversion.",
          "Default to separate HTML, CSS, JavaScript ES modules with explicit imports, and assets. Search selected-chain registry/index modules first and verify compatible candidates before reuse. Preserve module identities through publication and compress resources independently. Only make a self-contained file when the user explicitly requests it; Inline/onchain storage does not imply bundling.",
          "For collector-facing Inline, validate each supplied image, prepare its exact data:image/<type>;base64,<payload> carriage once at build time, and publish one receipt-bound ASCII payload or URI. Never publish raw image bytes plus a second encoded copy; the contract/viewer only copies the prepared header/payload/footer and never Base64-encodes media during tokenURI. GIF is direct data:image/gif from the exact high-quality source and is never wrapped in SVG, silently resized/re-encoded or replaced by a short fixture such as AA==. Keep the outer JSON and complete HTML in raw-percent form.",
    ...
    messages: [{
      ...
      content: {
        ...
        text: [
          ...
          "Before any contract, collection, viewer, metadata, deployment or release work, call keel-contract-workflow-preflight first; it reads the target README and relevant docs and returns the required engine, exact-network, selected-chain module, edge-case, and contract-control sequence. Then call keel-engine-catalog, inspect the selected chain, and search selected-chain modules. At Studio staging, supply only creator resources/modules. For Inline work, call keel-inline-prepare without a carriage or presentationPolicy override so the automatic compact raw-percent saver uses collector-inline policy, prepares one exact image carriage before publication, and does not Base64-wrap the complete HTML or metadata. The default image is a validated direct data:image/* URI/payload; the contract/viewer copies its prepared header/payload/footer and never encodes or decodes media during tokenURI. For GIF this is a direct high-quality GIF URI, never an SVG wrapper, silent resize/re-encode or placeholder. The default animation is complete data:text/html;charset=utf-8 HTML inside the registered canonical shell; IPFS, HTTP, web3 resolvers and legacy complete-HTML Base64 are explicit exceptions. Put executable reusable libraries in modules and artwork, animation, palettes, timing, and project data in assets; never embed a large Base64 artwork payload in entry HTML. Report original source, prepared image carriage, stored graph, complete tokenURI bytes, layer count, and overhead before staging, and fail above the public-read ceiling. Resolve the exact selected-chain KeelRawTokenURIBuilder plus canonical raw-percent shell fragment receipts and read-back; never fall back to legacy Base64 carriage silently. Omitting viewer selects Studio's canonical KEEL Inline graph for later preparation; `none` opts out of the shell only, while the immutable artifact can still be released, minted, and retrieved by its contract read. A standalone image, video, or self-contained GLB resolves to registered shell plus registered keel.asset-display@1 plus the direct creator asset, never zero modules or a manufactured index.html. Never use protectorPrefix, protectorSuffix, protectedHarnessDataURI, or a NoProtector result as the default Inline readiness check. Never manufacture or upload a default KEEL shell, protected-harness wrapper, or local wrapper when catalog resolution fails; Studio must fail closed for an incomplete selected-chain catalog during preparation. Creator-authored HTML is project content, not a replacement shell.",
          "Report unavailable carriers and modeled estimates honestly, and never call a URI proof of bytes.",
          "Before staging, unpack every embedded gzip or deflate resource and reject concrete http(s), IPFS, Arweave, web3 or keel-onchain locators in decoded creator bytes. An onchain content URL sentinel is not exempt; allow only the SVG namespace literal http://www.w3.org/2000/svg.",
          objectLine,
          mediaLine,
        ].join(" "),
      },
