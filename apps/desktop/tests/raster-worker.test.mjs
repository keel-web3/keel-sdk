...
test('editor worker exports exact PNG strips and APNG in isolated local folders',async()=>{
 const directory=await mkdtemp(path.join(os.tmpdir(),'keel-raster-worker-')),service=new RasterPrepareService(path.resolve('apps/desktop/dist/raster-prepare-worker.mjs'));
