...
function seedManifest(root, unit, deployable) {
  ...
  writeFileSync(join(unitDir(root, unit), "keel.module.json"),
    `${JSON.stringify({ schema: "keel.module@1", id: unit.id, kind: unit.kind, visibility: "private", tezos: { network: "ghostnet" }, deployable }, null, 2)}\n`);
}
...
test("regression: syncing an unbuilt external app does not empty its deployable list", (t) => {
  ...
  for (const unit of EXTERNAL) {
    const afterManifest = readManifest(root, unit);
    const after = afterManifest.deployable;
    ...
    assert.deepEqual(afterManifest.tezos, { network: "ghostnet" }, `${unit.id} lost its optional Tezos metadata`);
  }
